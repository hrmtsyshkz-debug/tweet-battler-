...
export function encodeOgData(winner: Fighter, loser: Fighter, finishingMove: string | null, winnerBadgeTier: number): string {
  const data: OgData = {
    wn: winner.name,
    wt: winner.typeKey,
    wh: winner.hp,
...
export function decodeOgData(encoded: string): { winner: Fighter; loser: Fighter; finishingMove: string | null; winnerBadgeTier: number } | null {
  try {
    ...
    return { winner, loser, finishingMove: data.fm || null, winnerBadgeTier: data.bt || 0 };
  } catch {
    return null;
  }
}
